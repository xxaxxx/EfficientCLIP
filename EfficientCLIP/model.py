...
class EfficientCLIP(nn.Module):
    ...
    # The forward step for contrastive learning tasks adding mlm task.
    def forward_mlm(self, image, text, img_queue, steps,queue_len, pad_id):
        ...
        loss_1, img_queue = self.criterion(zh_features, img_features, temp, img_queue, steps,queue_len=queue_len)
    ...
    # The forward step for only contrastive learning tasks.
    def forward(self, image, text, img_queue,queue_len, steps):
        
        # get the text features and text sequence embeddings
        text_features, zh_features = self.encode_text(text, boolean=True)

        #caculte the loss
        temp = self.logit_scale.exp()
        loss_1, img_queue = self.criterion(zh_features.to(torch.float32), image, temp, img_queue, steps,query_len=query_len)
        loss = loss_1
